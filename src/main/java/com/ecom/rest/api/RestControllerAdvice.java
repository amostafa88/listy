...
/**
 * 
 * Controller Advise . You create a standard class, configure it to contollers or all, and whenever an exception occure, this class will handle it, any thing has a rest api contoller annotation or subpackage, or exteindg from BaseAPI controller.  (OR)
 * it can be applied on packages with : basePackages="com.ecom", or type assignableTypes=BaseApiController.class)
 * 
 *  @author Adib Saikali
 *
 */
...
public class RestControllerAdvice
{
	...
	public ResponseEntity<RestApiError> handleHttpRequestMethodNotSupportedException(HttpRequestMethodNotSupportedException e)
	{
		logger.error("Api Error caused by exception", e);
		RestApiError restApiError = new RestApiError(RestApiHttpStatus.BAD_REQUEST);
		restApiError.setApiCode(ApiErrorCodes.METHOD_NOT_SUPPORTED_EXCEPTION);
